...
- **Per-environment facts are parameters, not properties.** `EnvironmentContext` (name, database, site map, media roots) is what `saveEntryForSites()`, the asset resolvers and every adapter take; the run's accumulators live on `RunTally`. The one module still *written onto* per environment is the CKEditor rewriter (its connection, caches and asset resolver), and `EnvironmentPipeline::adoptEnvironment()` is the only place that writes it — the reason a cache once outlived its database.
