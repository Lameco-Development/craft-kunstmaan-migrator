 ...
 */
class VerifyWorkflow extends Component
{
    use NeverProductionTrait;

    public bool $captureBaseline = false;
    public bool $captureBaselineHtml = false;
    ...
    /**
     * Run shared verify/report orchestration for CLI, CP, and queue adapters.
     *
     * Supported option keys are exactly baseline, urlSpotCheck, baselineDir,
     * countTolerance, urlDiffThreshold, entities, locales, since,
     * captureBaseline, captureBaselineHtml, output, and outputDir. Count gate
     * behavior remains delegated to CountGateService and baseline capture to
     * BaselineCounterService through the plugin service graph.
     *
     * @param array<string, mixed> $options
     * @param null|callable(array<string, mixed>): void $progress
     * @return array{status:string, artifactPaths:list<string>, summary:array<string,mixed>, failure:?array<string,mixed>}
     */
    public function run(array $options, ?callable $progress = null): array
    {
        $allowed = ['baseline', 'urlSpotCheck', 'baselineDir', 'countTolerance', 'urlDiffThreshold', 'entities', 'locales', 'since', 'captureBaseline', 'captureBaselineHtml', 'output', 'outputDir'];
        $unknown = array_values(array_diff(array_keys($options), $allowed));
        if ($unknown !== []) {
            throw new \InvalidArgumentException('Unsupported verify workflow option(s): ' . implode(', ', $unknown));
        }
        ...
        foreach ($allowed as $key) {
            ...
            if (in_array($key, ['baseline', 'urlSpotCheck', 'baselineDir', 'entities', 'locales', 'since', 'output', 'outputDir'], true)) {
                $this->{$key} = $options[$key] === null ? null : (string) $options[$key];
                continue;
            }
            if (in_array($key, ['countTolerance', 'urlDiffThreshold'], true)) {
                $this->{$key} = $options[$key] === null ? null : (float) $options[$key];
                continue;
            }
        ...
        try {
            if ($this->captureBaselineHtml) {
                $exitCode = $this->actionCaptureBaselineHtml();
            } elseif ($this->captureBaseline) {
                $exitCode = $this->actionCaptureBaseline();
            } else {
