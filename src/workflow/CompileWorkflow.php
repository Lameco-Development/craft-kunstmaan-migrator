 ...
 */
class CompileWorkflow extends Component
{
    ...
    /**
     * Run shared compile orchestration for CLI, CP, and queue adapters.
     *
     * Supported option keys are exactly overwrite and dryRun.
     *
     * @param array<string, mixed> $options
     * @param null|callable(array<string, mixed>): void $progress
     * @return array{status: string, artifactPaths: list<string>, summary: array<string, mixed>, fatalWarningCount: int, warningCount: int, compiledAt: ?string, failure: ?array<string, mixed>}
     */
    public function run(array $options, ?callable $progress = null): array
    {
        $allowed = ['overwrite', 'dryRun'];
        $unknown = array_values(array_diff(array_keys($options), $allowed));
        if ($unknown !== []) {
            throw new InvalidArgumentException('Unsupported compile workflow option(s): ' . implode(', ', $unknown));
        }

        $this->overwrite = (bool) ($options['overwrite'] ?? false);
        $this->dryRun = (bool) ($options['dryRun'] ?? false);
        $this->progress = $progress;
        $this->stdoutBuffer = [];
        $this->stderrBuffer = [];
        $this->artifactPaths = [];
        $this->fatalWarningCount = 0;
        $this->warningCount = 0;
        ...
        try {
            ...
            return [
                ...
                'fatalWarningCount' => $this->fatalWarningCount,
    ...
    private int $fatalWarningCount = 0;

    private int $warningCount = 0;

    private ?string $compiledAt = null;
    ...
    private function runExitCode(): int
    {
        ...
        $this->fatalWarningCount = count($graphFatalRows);
        $this->warningCount = count(array_filter(
            $graphCompatibilityRows,
            static fn(array $row): bool => (string) ($row['severity'] ?? '') !== 'fatal',
        ));
