...
| 08.6 | Page-builder block completeness (homepage unblocker) | Tighten 8.3's parent-blind block-type pick into parent-aware Matrix selection (when the same blockType handle exists on multiple Matrix fields, route by parent page's pageBuilderHandle). Add a per-block-type sub-field catalog so the LLM column proposer sees each pagePart's own block-fields when proposing column → block-field mappings. Add a per-pagepart column proposer step so each pagePart's residual columns get their own LLM batch (instead of being lumped under the parent page's entry-type). The CQM HomePage is the canonical case — it's the only page wrapping real page-parts into the page builder, so this phase is the last gate before homepage migration is complete. | (TBD — discuss-phase pending) | TBD | no |
| 08.7 | Page-rooted leaf-entity migration (embed vs promote) | Per-relation `mode: embed | promote` choice in mapping.yaml. Default `embed` keeps 8.5's `_rel:<prop>.<col>` inline behaviour. `promote` migrates the leaf entity (Employee, NewsCategory, NewsAuthor, …) as its own Craft entry-type, then sets a relation field on the wrapping page using the leaf's Craft id resolved from the migration state table. Discovery is page-rooted: only leaves reachable FROM pages/page-parts get migrated; leaves referenced by N pages dedupe via state (one Craft entry, N relation links). New `relation` handler at transform time consumes `_rel:<prop>` (property name, not dotted column) as the source. Topological extraction order ensures leaves migrate before their wrapping pages. ManyToMany / recursive joins out of scope (Phase 8.8+). | (TBD — discuss-phase pending) | TBD | no |
