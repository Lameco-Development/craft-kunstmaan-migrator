...
- [~] **CONN-03**: `kunstmaan-migrator/doctor` command reports OK/FAIL on: legacy DB reachability, Anthropic key presence, mapping file validity (if present), write permissions on `storage/migration/`. No queue-worker check — v1's check was carried by v1's queue-heavy pipeline; v2 is CLI-inline by default. _(Phase 1 / Plan 04 — partial: 3 of 4 checks ship per D-17 (legacyDb SELECT 1, anthropicApiKey presence-only-via-Settings-or-env, storage/migration writability with D-18 auto-create at 0755). The mapping-file check defers to Phase 2 alongside the mapping loader/validator; this requirement's wording will be patched by Plan 05 to acknowledge the 3-vs-4 delta. Plain-text OK/FAIL output with ANSI colors; exit 0 on full pass / 1 on any FAIL.)_
