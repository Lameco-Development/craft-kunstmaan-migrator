...
- [x] **SRC-13**: `KnowledgeBase` Markdown generator ported (partial — `renderPagesMarkdown()` + `renderPagePartsMarkdown()` + every transitively-reachable private helper per Plan 03's B3 anti-pattern guard) and wired into `LlmClassifier::batchPropose` via the SRC-09 v1-shaped adapter. _(Phase 02.1 / Plan 03 + Plan 07.)_
- [x] **SRC-14**: `BodyScanColumnFinder` + `MediaFkScanner` + `TopologicalOrderer` ported as part of the scanner output (composed by `KunstmaanSourceScanner`). Phase 3 consumes them — Phase 02.1 owns the discovery surface. _(Phase 02.1 / Plan 03 + Plan 05.)_
