...
| `sites` | object | Keyed by Craft site handle. Exactly the locales the legacy node has a translation the compiler carries, never more — a site the payload does not name must stay unnamed, because an empty row is what lets Craft propagate the primary site's slug into that locale. |
| `sites.*.enabled` | bool | Whether the entry is enabled for this site. False on a structural placeholder, and on a translation the legacy site had switched off that `defaults.offlineCutoff` rescued. |
